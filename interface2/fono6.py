...
class JanelaAvaliacaoPaciente(QDialog):
    ...
    def abrir_dados_recentes(self):
        ...
        if paciente_selecionado:
            dataframe_pacientes = pd.read_pickle("dataframe_pacientes.pkl")
...
class JanelaDadosRecentes(QDialog):
    ...
    def initUI(self, nome_paciente, matricula_paciente):
        texto_superior = QLabel("Dados Recentes", self)
        texto_superior.setStyleSheet("font-size: 18px; font-weight: bold;")

        figura, tabela = plot_pacientes.dados_recentes(paciente=nome_paciente, matricula=matricula_paciente)

        figura_canvas = FigureCanvas(figura)
        figura_layout = QVBoxLayout()
        figura_layout.addWidget(figura_canvas)

        tabela_widget = QLabel(str(tabela))
        tabela_widget.setStyleSheet("font-size: 12px;")
        ...
        layout = QVBoxLayout()
        layout.addWidget(texto_superior)
        layout.addLayout(figura_layout)
