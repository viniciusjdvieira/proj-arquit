...
class JanelaTerceiraTela(QDialog):
    ...
    def gravar_audio(self):
        if not self.gravando:
            ...
            default_input_info = sd.query_devices(None, 'input')
            default_input_device = default_input_info['default']

            def callback(indata, frames, time, status):
                self.frames.append(indata.copy())

            with sd.InputStream(device=default_input_device, channels=channels, samplerate=fs, callback=callback):
