...
from plots_pacientes import dados_recentes
...
class JanelaDadosRecentes(QDialog):
    ...
    def initUI(self, nome_paciente, matricula_paciente):
        ...
        figura, df_tab_vert = dados_recentes(nome_paciente, matricula_paciente)

        # Criar uma nova instância de Figure e passar a figura como subgráfico
        fig = Figure(figsize=(10, 4))
        sub_plot = fig.add_subplot(111)
        sub_plot.imshow(figura)

        # Criar um FigureCanvas com a nova instância de Figure
        figura_canvas = FigureCanvas(fig)

        tabela_label = QLabel("Valores médios", self)
        tabela_label.setStyleSheet("font-size: 16px; font-weight: bold;")

        tabela_widget = QTableView(self)
        tabela_model = pandas_util.PandasModel(df_tab_vert)
        tabela_widget.setModel(tabela_model)
        ...
        layout.addWidget(tabela_label)
        layout.addWidget(tabela_widget)
        layout.addWidget(voltar_button)
