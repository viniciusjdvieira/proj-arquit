...
class JanelaTerceiraTela(QDialog):
    def __init__(self, tela_anterior, numero_matricula):
        ...
        self.arquivo_gravacao = f"grav_{self.numero_matricula}.wav"  # Nome do arquivo de gravação

    def init_ui(self):
        mensagem_label = QLabel("Agora vamos gravar sua voz. \nClique no botão abaixo para começar a gravar e\n clique no botão de parar após 5 segundos. Você pode ouvir o áudio.", self)
        ...
        gravar_novamente_button.clicked.connect(self.gravar_novamente)
    ...
    def gravar_audio(self):
        if not self.gravando:
            ...
            # Verificar e excluir a gravação anterior, se existir
    ...
    def ouvir_audio(self):
        if os.path.exists(self.arquivo_gravacao):
            data, fs = sf.read(self.arquivo_gravacao, dtype='float32')
            sd.play(data, fs)
