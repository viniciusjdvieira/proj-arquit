...
class JanelaTerceiraTela(QDialog):
    ...
    def abrir_quarta_janela(self):
        self.quarta_janela = JanelaQuartaTela(self, self.frames, self.numero_matricula)
        self.quarta_janela.show()

###------------------------------

# Função para gerar a figura da forma de onda
...
# Função para gerar a figura do espectrograma
...
class JanelaQuartaTela(QWidget):
    def __init__(self, tela_anterior, frames, numero_matricula):
        super().__init__(tela_anterior)
        self.setWindowTitle("Quarta Janela")
        self.setStyleSheet("background-color: darkgreen; color: white;")
        ...
        # Mensagem
        mensagem_label = QLabel("Sua voz foi analisada", self)
        layout.addWidget(mensagem_label)

        # Forma de Onda
        waveform_fig = plot_waveform(frames, sample_rate=22050)
        waveform_canvas = FigureCanvas(waveform_fig)
        layout.addWidget(waveform_canvas)

        # Espectrograma
        spectrogram_fig = plot_spectrogram(frames, sample_rate=22050)
